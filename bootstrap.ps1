...
param(
    ...
    [switch]$NoColor,
    [switch]$Ascii,
    # Passed through to Invoke-GuidePlan when guide is active: emit JSON to stdout
    # instead of TUI/streaming output. Used by test-guide-integration.ps1 and CI.
    [switch]$Headless
)
...
# ── Guide integration (subsequent runs only) ─────────────────────────────
...
# block activates the Guide TUI.
$guideModule = Join-Path (Split-Path -Parent $scriptRoot) "tools/guide/guide.psd1"
$useGuide = $false

if (Test-Path $guideModule) {
    try {
        Import-Module $guideModule -Force -ErrorAction Stop
        $useGuide = $true
    } catch {
        Write-Warning "(Guide module exists but failed to load: $_. Falling back to streaming renderer.)"
    }
}
# ── End guide detection ──────────────────────────────────────────────────
...
# ── Build-BootstrapPlan — maps existing phases to Guide plan schema ──────
# Called only when $useGuide is $true. Defined here so lib functions are
# available (dot-sourced above). Placed before $if ($Help) so the guide
...
function Build-BootstrapPlan {
    ...
    # Promote ParentDir to script scope so the action scriptblocks below — which
    # are invoked later by guide, after this function has returned — can still
    # resolve it. (PowerShell scriptblocks aren't closures by default; using
    # .GetNewClosure() would capture the variable but sever access to the
    # script-scoped helper functions dot-sourced from lib/.)
    $script:ParentDir = $ParentDir

    $plan = New-GuidePlan -Title "Fellwork Bootstrap" -Subtitle (Get-Date -Format "HH:mm:ss") -Theme "twilight"

    # ── Phase 1: Bootstrap prerequisites ─────────────────────────────────────
    $plan = Add-GuideSinglePhase -Plan $plan -Name "Bootstrap prerequisites" -Action {
        ...
        foreach ($p in $bootstrapPrereqs) {
            ...
            if ($r.Status -eq 'present') {
                $alerts += New-GuideAlert -Severity 'info' -Message "$p $($r.Version)"
            } else {
                $allOk = $false
                $alerts += New-GuideAlert -Severity 'fail' `
        ...
        return New-GuideResult -Success $true -Message "Bootstrap prerequisites satisfied" -Alerts $alerts
    }

    # ── Phase 2: Clone repos ──────────────────────────────────────────────────
    $plan = Add-GuideLoopPhase -Plan $plan -Name "Cloning repositories" -Items $repoData.repos -Action {
        param($repo)
        $expectedOrigin = "https://github.com/$($repo.org)/$($repo.name).git"
        $state = Get-RepoState -Name $repo.name -ParentDir $script:ParentDir -ExpectedOrigin $expectedOrigin
        ...
        switch ($state.Status) {
            ...
            'present-mismatch' {
                return New-GuideResult -Success $false `
                    ...
                    -Severity 'fail' `
                    -FixCommand "cd $script:ParentDir/$($repo.name) && git remote set-url origin $expectedOrigin" `
                    -Animal 'raccoon'
            }
            'present-not-git' {
                return New-GuideResult -Success $false `
            ...
            'absent' {
                $clone = Invoke-RepoClone -Name $repo.name -Org $repo.org -ParentDir $script:ParentDir
                if ($clone.Success) {
                    return New-GuideResult -Success $true -Message "$($repo.name): cloned"
                } else {
                    return New-GuideResult -Success $false `
    ...
    $plan = Add-GuideLoopPhase -Plan $plan -Name "Toolchain (proto install per repo)" -Items $okRepos -Action {
        param($repo)
        $repoPath = Join-Path $script:ParentDir $repo.name
        $result = Invoke-ProtoInstall -RepoPath $repoPath

        if ($result.Skipped) {
            return New-GuideResult -Success $true -Message "$($repo.name): no .prototools — skipped" -Severity 'info'
        } elseif ($result.Success) {
            return New-GuideResult -Success $true -Message "$($repo.name): tools installed"
        } else {
            return New-GuideResult -Success $false `
    ...
    $plan = Add-GuideSinglePhase -Plan $plan -Name "Other prerequisites" -Action {
        ...
        foreach ($p in $otherPrereqs) {
            if ($p.Status -eq 'present') {
                $alerts += New-GuideAlert -Severity 'info' -Message "$($p.Name) $($p.Version)"
            } elseif ($p.Status -eq 'present-but-not-running') {
                $fixCmd = if ($p.Workaround) { $p.Workaround } else { "Start the $($p.Name) daemon (e.g. open Docker Desktop)" }
                $alerts += New-GuideAlert -Severity 'warning' `
        ...
        return New-GuideResult -Success $true -Message "Other prerequisites checked" -Alerts $alerts
    }
    ...
    if ($reposNeedingEnv.Count -gt 0) {
        $plan = Add-GuideLoopPhase -Plan $plan -Name "Env files" -Items $reposNeedingEnv -Action {
            param($repo)
            $needed = Test-EnvFilesNeeded -Repo $repo -ParentDir $script:ParentDir
            $alerts = @()
            $anyFail = $false
            ...
            return New-GuideResult -Success $true -Message "$($repo.name): env files scaffolded" -Alerts $alerts
        }
    }

    # ── Phase 6: Structure validation ─────────────────────────────────────────
    $plan = Add-GuideLoopPhase -Plan $plan -Name "Repo structure validation" -Items $okRepos -Action {
        param($repo)
        $valid = Test-RepoStructure -Repo $repo -ParentDir $script:ParentDir

        if ($valid) {
            return New-GuideResult -Success $true -Message "$($repo.name): structure OK"
        } else {
            $expected = if ($repo.structureCheck) { $repo.structureCheck -join ', ' } else { '(check repos.psd1)' }
            return New-GuideResult -Success $false `
                -Message "$($repo.name): expected files missing" `
                -Severity 'warning' `
                -FixCommand "cd $script:ParentDir/$($repo.name) && check that these files exist: $expected" `
                -Animal 'raccoon'
        }
    }
...
# ── Guide early-exit (replaces streaming output for the entire run) ──────
# Runs after --help so that -Help always works regardless of tools/ state.
# Runs before streaming setup so no streaming output is emitted on the guide path.
if ($useGuide) {
